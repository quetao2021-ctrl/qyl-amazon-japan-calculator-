...
const wfPath = 'E:/codex/n8n_gemini_workflows/gemini_web_rpa_7plus4_workflow_zh.json';

const buildCommandCode = `
const pick = (...keys) => {
  for (const k of keys) {
    const v = $json[k];
  ...
  nodes: [
    {
      parameters: {
        ...
        formDescription: '先在浏览器登录Gemini，再提交。工作流会按 7 主图 + 4 A+ 图执行。',
        formFields: {
          values: [
    ...
    {
      parameters: { jsCode: buildCommandCode },
    ...
    {
      parameters: { jsCode: extractResultCode },
...
fs.writeFileSync(wfPath, JSON.stringify(workflow, null, 2), { encoding: 'utf8' });
