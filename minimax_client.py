...
DEFAULT_TIMEOUT_SECONDS = float(os.getenv("MINIMAX_TIMEOUT_SECONDS", "60"))
...
class MiniMaxClient:
    def __init__(
        ...
        key = api_key or os.getenv("MINIMAX_API_KEY") or os.getenv("OPENAI_API_KEY")
        if not key:
            raise ValueError(
                "Missing API key. Set MINIMAX_API_KEY (recommended) or OPENAI_API_KEY."
            )
        if max_retries < 0:
    ...
    def _with_retries(self, call: Callable[[], Any]) -> Any:
        ...
        while True:
            try:
                ...
                retriable = status in {408, 409, 429} or (status is not None and status >= 500)
